document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchJSONData() {
      const response = await fetch('https://content.jusaninvest.kz/pifs/by-slug/harmoney');
    ...
    console.log(jsonData, ' json data ')
        ...
        jsonData.assetsAllocation.pieChart1.forEach((item, index) => {
          ...
          numberDiv.id = `harmoney-${item.id}`; // Use the item's ID as part of the ID
    ...
    jsonData.portfolioStructure.forEach((item, index) => {
     ...
     // Дата
     const harmoneyDateString = jsonData.income.date + ' ж деректер';
     const harmoneyDateElement = document.getElementById('harmoney-date-kz');
     harmoneyDateElement.innerHTML = harmoneyDateString;
...
pifPriceElement.innerHTML = trimmedPifPriceString
    ...
    const priceDateElement = document.getElementById('harmoney-price-date')
    priceDateElement.innerHTML = priceDateString
    // 1 месяц
    const oneMonthString = jsonData.income.oneMonth
    const oneMonthElement = document.getElementById('harmoney-one-month-with-dividends')
    ...
    const threeMonthsElement = document.getElementById('harmoney-three-months-with-dividends')
    ...
    const halfYearElement = document.getElementById('harmoney-half-year-with-dividends')
    ...
    const yearElement = document.getElementById('harmoney-year-with-dividends')
    ...
    const fromBeginningElement = document.getElementById('harmoney-from-beginning-with-dividends')
